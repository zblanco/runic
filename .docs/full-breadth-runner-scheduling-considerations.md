...
The execution pipeline now follows a **three-phase event-driven model** (prepare → execute → apply):
...
│               → prepare/3 produces {workflow, [%Runnable{}]} │
...
│               apply_runnable/2 folds events via           │
│               apply_event/2 into the workflow graph.      │
│               Events are accumulated in                   │
│               workflow.uncommitted_events.                │
│  5. REPEAT:   if is_runnable?(wf), goto 2                 │
│                                                           │
│  Persistence: maybe_checkpoint →                          │
│    Event-sourced path: Store.append/3 (drains             │
│      uncommitted_events)                                  │
│    Legacy path: Store.save/3                              │
│  Telemetry:   Telemetry.runnable_event(...)               │
└──────────────────────────────────────────────────────────┘
```

The core insight is that **step 4 (Apply)** now produces a stream of typed events (11 built-in event types: `FactProduced`, `ActivationConsumed`, `RunnableActivated`, `ConditionSatisfied`, `MapReduceTracked`, `StateInitiated`, `JoinFactReceived`, `JoinCompleted`, `JoinEdgeRelabeled`, `FanOutFactEmitted`, `FanInCompleted`) rather than performing opaque mutations. These events are folded into the workflow graph via `apply_event/2` and accumulated in `uncommitted_events`. The Worker drains these events during checkpointing via `Store.append/3` (event-sourced path) or falls back to `Store.save/3` (legacy full-state path). Workflows can be rebuilt from an event stream via `Workflow.from_events/2`.

**Steps 3 and 4** — dispatch and apply — remain where all extensibility opportunities live. The plan and prepare phases are pure data transformations on the `Workflow` struct. The dispatch phase decides _how_ and _where_ runnables execute. The apply phase now produces and folds events, making execution mutations explicit and replayable. Everything between prepare and apply is the **execution gap** that this document addresses.
...
**Note on the event-sourced model and remote execution:** The three-phase execution model makes remote execution more natural than the closure-based approach. The Execute phase is isolated and produces events. A remote Executor could serialize the `%Runnable{}`, execute it remotely, and return the resulting events. The coordinating Worker then folds those events into its local workflow via `apply_event/2`. Events are serializable via ETF (see `Runic.Workflow.Events.Serializer`). This cleanly decouples execution from state management — the remote side only needs to invoke the runnable and capture events, while the local Worker owns the workflow graph and applies the mutations.
...
**Critical insight:** Inside a Promise, `apply_runnable/2` folds events via `apply_event/2` on the local workflow copy. When `emit_events` is true, the local copy accumulates `uncommitted_events` as each intermediate runnable completes. When the Promise resolves, the Worker receives all completed runnables and applies them to the source-of-truth workflow — draining the accumulated `uncommitted_events` during the next checkpoint cycle. This is a natural consequence of the event-driven model, not a special case: every `apply_runnable/2` call produces events regardless of whether it runs inside a Promise or directly in the Worker.
...
**Note on event-sourced model impact:** Phase A (Executor Behaviour) and the checkpointing integration are now less coupled because event-sourced checkpointing is already in place. The Worker's `do_checkpoint/1` already handles event draining via `Store.append/3`. Phase C (Promise Model) checkpoint semantics are simpler — Promises accumulate `uncommitted_events` during resolution, and the Worker drains them on Promise completion. This removes what was previously a blocking dependency between the Promise model and checkpointing design.
...
**What Runic has adopted:**
- Runic's event stream is now analogous to Restate's journal — both are append-only records of execution mutations. The Worker drains `uncommitted_events` via `Store.append/3`, and workflows can be rebuilt from the event stream via `Workflow.from_events/2`.
- Unlike Restate, Runic's events fold into a graph model (not a linear journal), which enables richer coordination patterns (Join, FanIn, state machines). Restate journals replay linearly; Runic's `apply_event/2` folds events into a multigraph DAG that preserves causal structure.

**What Runic can still learn:**
- Restate's journal is compactable. Runic should pursue snapshot + WAL compaction to match this property.
...
**What Runic has adopted:**
- `Workflow.from_events/2` with a base workflow is directly analogous to Azure's "replay from checkpoint" model. Both systems re-execute by replaying a log of mutations against a base state, and both skip re-execution of already-completed activities/runnables.
...
- Azure's "continue-as-new" pattern (restart with fresh history when it grows too large) could be implemented as: snapshot current state → start fresh event stream. The event-sourced model makes this natural — `save_snapshot/4` captures the current graph, and a new event stream begins from that point.
...
| **Runic** | **Event-Sourced Functional DAG** | **None assumed** | **Behaviours at every layer** |
...
**Status: Core mechanism IMPLEMENTED.** Incremental checkpointing is now in place via the event-sourced model. The Worker already routes between `Store.append/3` (event-sourced, drains `uncommitted_events`) and `Store.save/3` (legacy full-state) paths in `do_checkpoint/1`. The Store behaviour defines `append/3`, `stream/2`, `save_snapshot/4`, `load_snapshot/2`, `save_fact/3`, and `load_fact/2`.

What remains as future work:
- **Snapshot + WAL strategy** for faster recovery: periodically take a snapshot (`save_snapshot/4`) and replay only events after the snapshot, rather than replaying the full event stream via `from_events/2`.
- **Fact-level content-addressed storage** integration: `FactRef` is defined but not yet integrated into the graph. Content-addressed facts would enable deduplication and efficient storage of large intermediate results.
- **Hybrid rehydration** for memory-bounded recovery: combine snapshots, event replay, and lazy fact loading to bound memory during workflow reconstruction.

Promise checkpoint semantics are simpler under this model — Promises accumulate `uncommitted_events` during resolution, and the Worker drains them on Promise completion during the normal checkpoint cycle. No special-casing is required.
...
13. **Event capture completeness:** The current `apply_runnable/2` captures core events and coordination events in `uncommitted_events`, but downstream activation edges (drawn by `emit_downstream_activations` and `Activator.activate_downstream`) are graph mutations NOT captured as events. For the Executor behaviour and remote execution, should all graph mutations during the apply phase produce events? This would make the event stream a complete, replayable record of all state transitions — critical for remote Executors and distributed Workers that need to synchronize state. The tradeoff is event volume: activation edges are numerous (every downstream connection from a produced fact), and capturing them all would significantly increase the event stream size for large workflows.

### Layering

14. **Opt-in complexity:** The layered model (Executor → Scheduler Strategy → Promise → Worker) introduces many new concepts. How do we ensure users at Layer 4 (standard Runner) never need to think about Promises or Executor behaviours unless they want to? The answer is strong defaults: `TaskExecutor` + `DefaultScheduler` + no Promises. But we need to verify that adding these layers doesn't introduce overhead when they're not used.

15. **Testing surface:** Each layer multiplies the testing matrix. The key combinatorial dimensions are: `{Executor} × {Scheduler Strategy} × {Checkpoint Strategy} × {Store Adapter} × {Workflow Shape}`. Should we define a conformance test suite for each behaviour (like the Store contract tests) that adapter authors can run to verify their implementation?
